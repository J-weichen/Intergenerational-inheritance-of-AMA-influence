...
single_kids_200bin_data<-read.csv("/mnt/data/chenwei/huahua/4.methy_result/0.global_compasion/single_kids_20_samples_meth_level_CpG_6X_meth_bismark_200bp_3L_8sample_meth_PVALUE.csv",row.names = 1,check.names=F)
head(single_kids_200bin_data);dim(single_kids_200bin_data)
#756666     26

##test evaluation
str(single_kids_200bin_data)
myDiffq005_all<-single_kids_200bin_data[single_kids_200bin_data$qvalue <0.05,]
dim(myDiffq005_all)#164059     26
...
pdf("/mnt/data/chenwei/huahua/4.methy_result/1.DMR_called/single_kids_AMA_vs_Young_200bin_q005_dif15_distribution.pdf")
...
# -42.1761  -2.5717  -1.2128  -0.6543   1.6637  42.4768 
summary(myDiffq005_all[myDiffq005_all$group1=="hyper",]$meth.diff)
#      Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  0.1467  1.4776  2.3358  2.9613  3.7040 42.4768 
summary(myDiffq005_all[myDiffq005_all$group1=="hypo",]$meth.diff)
#  Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#-42.1761  -3.4161  -2.1182  -2.7119  -1.3164  -0.1457 

library(psych)
describe(myDiffq005_all$meth.diff)
##非缺失值的数量、平均数、标准差、中位数、截尾均值、绝对中位差、最小值、最大值、值域、偏度、峰度和平均值的标准误
##vars     n    mean    sd median trimmed   mad    min   max  range skew kurtosis   se
##  X1    1 164059 -0.65 3.53  -1.21   -0.71 3.14 -42.18 42.48 84.65 0.16     4.12 0.01

describe(myDiffq005_all[myDiffq005_all$group1=="hyper",]$meth.diff)
#vars     n     mean    sd median trimmed  mad  min   max range skew kurtosis   se
## X1    1 59501 2.96 2.31   2.34    2.59 1.5 0.15 42.48 42.33 2.76    14.54 0.01

describe(myDiffq005_all[myDiffq005_all$group1=="hypo",]$meth.diff)
#vars        n   mean   sd median trimmed  mad    min   max range  skew kurtosis   se
##X1    1 104558 -2.71 2.19  -2.12   -2.36 1.42 -42.18 -0.15 42.03 -2.85    16.21 0.01

pdf("/mnt/data/chenwei/huahua/4.methy_result/1.DMR_called/single_kids_AMA_vs_Young_Diff_200bp_DMR_pie50_25_20_10_5_def_plot.pdf")
...
single_kids_200bin_data$threshold = as.factor(ifelse(single_kids_200bin_data$qvalue < 0.05 & abs(single_kids_200bin_data$meth.diff)>= 50, ifelse(single_kids_200bin_data$meth.diff>= 50 ,'Hyper','Hypo'),'NoSignifi'))
table(single_kids_200bin_data$threshold)
#NoSignifi 
#  756666   
single_kids_200bin_data$threshold = as.factor(ifelse(single_kids_200bin_data$qvalue < 0.05 & abs(single_kids_200bin_data$meth.diff)>= 25, ifelse(single_kids_200bin_data$meth.diff>= 25 ,'Hyper','Hypo'),'NoSignifi'))
table(single_kids_200bin_data$threshold)
# Hyper      Hypo NoSignifi 
#  18        21    756627
single_kids_200bin_data$threshold = as.factor(ifelse(single_kids_200bin_data$qvalue < 0.05 & abs(single_kids_200bin_data$meth.diff)>= 20, ifelse(single_kids_200bin_data$meth.diff>= 20 ,'Hyper','Hypo'),'NoSignifi'))
table(single_kids_200bin_data$threshold)
#Hyper      Hypo NoSignifi 
# 54        78    756534 
single_kids_200bin_data$threshold = as.factor(ifelse(single_kids_200bin_data$qvalue < 0.05 & abs(single_kids_200bin_data$meth.diff)>= 15, ifelse(single_kids_200bin_data$meth.diff>= 15 ,'Hyper','Hypo'),'NoSignifi'))
table(single_kids_200bin_data$threshold)
#  Hyper    Hypo  NoSignifi 
#  226       298    756142 
single_kids_200bin_data$threshold = as.factor(ifelse(single_kids_200bin_data$qvalue < 0.05 & abs(single_kids_200bin_data$meth.diff)>= 10, ifelse(single_kids_200bin_data$meth.diff>= 10 ,'Hyper','Hypo'),'NoSignifi'))
table(single_kids_200bin_data$threshold)
#Hyper      Hypo    NoSignifi 
# 1049      1469    754148 

#Final usage:
compare_name<-"single_kids_AMA_vs_Young"
single_kids_200bin_data$threshold = as.factor(ifelse(single_kids_200bin_data$qvalue < 0.05 & abs(single_kids_200bin_data$meth.diff)>= 25, ifelse(single_kids_200bin_data$meth.diff>= 25 ,'Hyper','Hypo'),'NoSignifi'))
table(single_kids_200bin_data$threshold)
single_kids_200bin_data$threshold<-factor(single_kids_200bin_data$threshold,level=c("Hyper","Hypo","NoSignifi"))
p_df25<-ggplot(data = single_kids_200bin_data, aes(x = meth.diff, y = -log10(qvalue), colour=threshold)) +
...
table(single_kids_200bin_data$threshold)
single_kids_200bin_data$threshold<-factor(single_kids_200bin_data$threshold,level=c("Hyper","Hypo","NoSignifi"))
p_df20<-ggplot(data = single_kids_200bin_data, aes(x = meth.diff, y = -log10(qvalue), colour=threshold)) +
  geom_point(alpha=0.4, size=3.5) +
  scale_color_manual(values=c("red","blue","grey")) +
  scale_x_continuous(breaks=seq(-80, 80, 20))+
  ylim(0, 600)+
...
table(single_kids_200bin_data$threshold)
single_kids_200bin_data$threshold<-factor(single_kids_200bin_data$threshold,level=c("Hyper","Hypo","NoSignifi"))
p_df15<-ggplot(data = single_kids_200bin_data, aes(x = meth.diff, y = -log10(qvalue), colour=threshold)) +
  geom_point(alpha=0.4, size=3.5) +
  scale_color_manual(values=c("red","blue","grey")) +
  scale_x_continuous(breaks=seq(-80, 80, 20))+
  ylim(0, 600)+
...
myDiff15q005_all <- single_kids_200bin_data[which(single_kids_200bin_data$qvalue < 0.05 & abs(single_kids_200bin_data$meth.diff)>= 15), ]
