...
pdf("/mnt/data/chenwei/huahua/4.methy_result/1.DMR_called/mother_AMA_vs_Young_200bin_q005_dif15_distribution.pdf")
...
#    -37.9418  -2.7166  -1.3992  -0.7125   1.5808  30.1797 

summary(myDiffq005_all[myDiffq005_all$group1=="hyper",]$meth.diff)
#      Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  0.1431  1.3564  2.4119  3.0486  3.9761 30.1797 

summary(myDiffq005_all[myDiffq005_all$group1=="hypo",]$meth.diff)
#  Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#-37.9418  -3.5081  -2.2717  -2.8262  -1.4893  -0.1569 
library(psych)
describe(myDiffq005_all$meth.diff)
##非缺失值的数量、平均数、标准差、中位数、截尾均值、绝对中位差、最小值、最大值、值域、偏度、峰度和平均值的标准误
##vars     n    mean    sd median trimmed   mad    min   max  range skew kurtosis   se
##X1       1 147023 -0.71 3.62   -1.4   -0.83 3.03 -37.94 30.18 68.12 0.29     3.78 0.01

describe(myDiffq005_all[myDiffq005_all$group1=="hyper",]$meth.diff)
#vars     n     mean    sd median trimmed  mad  min   max range skew kurtosis   se
##X1    1 52898 3.05 2.49   2.41    2.66 1.81 0.14 30.18 30.04 2.34     9.76 0.01
describe(myDiffq005_all[myDiffq005_all$group1=="hypo",]$meth.diff)
#vars        n   mean   sd median trimmed  mad    min   max range  skew kurtosis   se
##X1    1 94125 -2.83 2.14  -2.27   -2.49 1.36 -37.94 -0.16 37.78 -3.01    18.11 0.01

pdf("/mnt/data/chenwei/huahua/4.methy_result/1.DMR_called/mother_AMA_vs_Young_Diff_200bp_DMR_pie50_25_20_10_5_def_plot.pdf")
