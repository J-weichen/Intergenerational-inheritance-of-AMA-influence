...
#  12.14       4.37      41.75      41.75 
#hypo
#promoter       exon     intron intergenic 
#   10.08       8.91      42.25      38.76 
#all
#promoter       exon     intron intergenic 
# 10.99       6.90      42.03      40.09 
...
annotation_DMR_hyper<-as.data.frame(peakAnnoList$DMRs_hyper)
range(annotation_DMR_hyper$distanceToTSS)# -880076  379299
...
annotation_DMR_hypo<-as.data.frame(peakAnnoList$DMRs_hypo)
range(annotation_DMR_hypo$distanceToTSS)#-767722  437380
...
#注释数据与位点甲基化水平合并
myDiff_data<- read.csv("/mnt/data/chenwei/huahua/4.methy_result/0.global_compasion/father_20_samples_meth_level_CpG_6X_meth_bismark_200bp_3L_8sample_meth_PVALUE.csv", header=T,sep=",",check.names = F)
head(myDiff_data)
...
data_region_merge$meth_df2<-(data_region_merge$AMA_father_mean -data_region_merge$YOUNG_father_mean)
data_region_merge$log10_FC<-log10(data_region_merge$AMA_father_mean/data_region_merge$YOUNG_father_mean)

head(data_region_merge,n=3);dim(data_region_merge)
length(which(abs(data_region_merge$meth.diff)>=20))#128
