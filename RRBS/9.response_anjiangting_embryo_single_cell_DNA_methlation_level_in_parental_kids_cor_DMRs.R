...
JTA_files<-list.files("/mnt/data/anjianting/project/devomics/human/preimplant/human.methylation/WCG.LiLin.bed/WCG.bed")
length(JTA_files)
JTA_files2<-as.list(paste0("/mnt/data/anjianting/project/devomics/human/preimplant/human.methylation/WCG.LiLin.bed/WCG.bed/",JTA_files))

sample_names<-c();interSE<-list()
for (file in JTA_files2) {
  
  #file<-"/mnt/data/anjianting/project/devomics/human/preimplant/human.methylation/WCG.LiLin.bed/WCG.bed/2cell_embryo1_1.ACG.TCG.bed.gz"                                                  
  #print(file)
  Sample_Name<-unlist(lapply(strsplit(file,"/"), function(x) x[12]))
...
saveRDS(interSE,file = "/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/JTA_single_Cell_pre_implantation_embryo_meth_data_in_two_corDMRs_extend_500bp.rds")
...
saveRDS(mymergeddata,file = "/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/data_merge_JTA_single_Cell_pre_implantation_embryo_meth_data_in_two_corDMRs_extend_500bp.rds")


#rm(list = ls())
mymergeddata<-readRDS(file = "/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/data_merge_JTA_single_Cell_pre_implantation_embryo_meth_data_in_two_corDMRs_extend_500bp.rds")
...
length(rownames(mymergeddata))#101

#show methlation level in heatmap in global
JTA_meta <-data.frame(sample=colnames(mymergeddata))
...
head(colData)
table(colData$stage)
#2cell  4cell  8cell    ICM Morula     TE Zygote 
#   14     26     48     50     25     48     10 

colData$stage<-factor(colData$stage,levels = c("Zygote","2cell","4cell","8cell","Morula","ICM","TE"))
...
head(mymergeddata2)
dim(mymergeddata2);dim(mymergeddata)#67 221 # 101 221
...
#Zygote  2cell  4cell  8cell Morula    ICM     TE 
#  10     14     26     48     25     50     48 
...
Stages<-c("Zygote","2cell","4cell","8cell","Morula","ICM","TE")
...
pdf("/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/JTA_DNA_methylation_level_of_DMRs_in_JTA_single_Cell_pre_implantation_embryo_heatmap_500bp_extend_1.pdf",width=16,height=8)
print(p1)
dev.off()
...
p1<-pheatmap(mymergeddata2, cluster_row =FALSE,cluster_col =FALSE,na_col = "gray",
             ...
             color = colorRampPalette(c("navy","white","orange","firebrick3"))(50),
             main ="DNA methylation level of DMRs in early embryo development(original value)",angle_col ="90")
pdf("/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/JTA_after_rm_DNA_methylation_level_of_DMRs_in_JTA_single_Cell_pre_implantation_embryo_heatmap_500bp_extend_1.pdf",width=16,height=8)
print(p1)
dev.off()
...
pdf("/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/JTA_after_rm_DNA_methylation_level_of_DMRs_in_early_embryo_development_heatmap_500bp_extend_2.pdf",width=16,height=8)
print(p2)
dev.off()
dev.new()

###perform cluster analysis based on stage average value
...
#mymergeddata<-mymergeddata[,rownames(na_number2)]
#colData3<-colData2[rownames(na_number2),]
#head(colData3)
#dim(mymergeddata);dim(colData3)
#write.table(mymergeddata, '/mnt/data/chenwei/huahua/manuscript/figure/figure4/table/na_rm_DNA_methylation_level_of_DMRs_in_early_embryo_development_for_correlated_DMR_Kid_parents.txt', sep = '\t', col.names = NA, quote = FALSE)
...
head(my_data2)
head(colData)
length(unique(my_data2$sample));length(unique(colData$sample))#221
my_data3<-merge(my_data2,colData,by="sample")
head(my_data3)
unique(as.character(my_data3$variable))
head(my_data3,n=7)
dim(my_data3)#22321     4
write.csv(my_data3,"/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/not_remove_data_merge_scDNA_methylation_level_of_DMRs_in_JTA_single_Cell_pre_implantation_embryo_in_two_corDMRs_add_meta_500bp_extend.csv")

##step five:  plot target regions DNA methylation
##for all region
plot_data <-read.csv("/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/not_remove_data_merge_scDNA_methylation_level_of_DMRs_in_JTA_single_Cell_pre_implantation_embryo_in_two_corDMRs_add_meta_500bp_extend.csv",row.names = 1)
head(plot_data,n=7)
...
embryo_plot2
ggsave("/mnt/data/chenwei/intergeneration_epigenetics/early_embryo/no_remove-box_meth_in_each_DMRs_for_JTA_single_Cell_pre_implantation_embryo_500bp_extend.pdf",embryo_plot,width=50, height=50,limitsize = FALSE)

##plot global DNA methylation of each sample in every stages
plot_data2 <-aggregate(x=plot_data_select3$value,by=list(plot_data_select3$sample,plot_data_select3$stage),FUN=mean, na.rm=TRUE, na.action=NULL)
head(plot_data2)
...
pdf("/mnt/data/chenwei/huahua/4.methy_result/3.cor_DMRs/no_remove_JTA_single_Cell_pre_implantation_embryo_boxplot_DNA_methylation_level_of_DMRs_heatmap.pdf",width=16,height=8)
plot1
plot2
dev.off()
...
#选择区域的分组求均值后进行归一化 
head(plot_data_select3)
plot_data2<-plot_data_select3[,c("variable","stage","value")]
head(plot_data2)
...
write.table(plot_matrix, '/mnt/data/chenwei/huahua/manuscript/figure/figure4/table/no_remove_JTA_single_Cell_pre_implantation_embryo_all_cor_DMRs_zscore_data_early_embryo_expression_time_cluster_pattern_for_correlated_DMR_Kid_parents.txt', sep = '\t', col.names = NA, quote = FALSE)
write.table(plot_data3, '/mnt/data/chenwei/huahua/manuscript/figure/figure4/table/no_remove_no_scale_JTA_single_Cell_pre_implantation_embryo_all_cor_DMRs_zscore_data_early_embryo_expression_time_cluster_pattern_for_correlated_DMR_Kid_parents.txt', sep = '\t', col.names = NA, quote = FALSE)

##DMR根据发育阶段变化的聚类plot 
data_expr<-read.table('/mnt/data/chenwei/huahua/manuscript/figure/figure4/table/no_remove_JTA_single_Cell_pre_implantation_embryo_all_cor_DMRs_zscore_data_early_embryo_expression_time_cluster_pattern_for_correlated_DMR_Kid_parents.txt', header = T,row.names = 1)
head(data_expr)
...
#there are NAs in some of the entries, indicating between some rows, it's not possible to calculate euclidean distances. 
#You need to the euclidean distance matrix to have no NAs to do clustering:
sum(is.na(as.matrix(dist(data_expr))))#182
...
head(data_expr2)
dim(data_expr2);dim(data_expr)#91  7 # 92  7


#plot_matrix<-data_expr[,-ncol(data_expr)]
colnames(data_expr2)<-c("Zygote","Two_cell","Four_cell","Eight_cell","Morula","ICM","TE")
data_expr2[1:4,]


anno_cell<-c("Zygote","Two_cell","Four_cell","Eight_cell","Morula","ICM","TE")
#anno_cell2<-c(rep("Zygote",1),rep("ZGA_pre",2),rep("ZGA_after",2),rep("Differention",2))
ann_colors = list(cell_stage=c(Zygote=ppCor[1],Two_cell=ppCor[2],Four_cell=ppCor[3],Eight_cell=ppCor[4],
                               Morula=ppCor[5],ICM=ppCor[6],TE=ppCor[7]) )#,
...
pdf("/mnt/data/chenwei/huahua/manuscript/figure/figure4/no_remove_JTA_single_Cell_pre_implantation_embryo_all_cor_DMRs_zscore_data_gamete_early_embryo_expression_time_cluster_pattern_for_correlated_DMR_Kid_parents_heatmaps.pdf",width = 6,height =6)
print(htkm0)
dev.off()
...
pdf("/mnt/data/chenwei/huahua/manuscript/figure/figure4/no_remove_JTA_single_Cell_pre_implantation_embryo_split_mean_cluster_pattern_for_correlated_DMR_Kid_parents_heatmaps.pdf",width = 8,height =8)
print(htkm)
dev.off()
...
trend_plot3
ggsave(file="/mnt/data/chenwei/huahua/manuscript/figure/figure4/no_remove_JTA_single_Cell_pre_implantation_embryo_median_methy_trends_for_different_cluster_gamete_early_embryo_expression_time_cluster_pattern_for_correlated_DMR_Kid_parents.pdf",trend_plot2,width =6, height =6,limitsize = FALSE)
ggsave(file="/mnt/data/chenwei/huahua/manuscript/figure/figure4/no_remove_JTA_single_Cell_pre_implantation_embryo_median_methy_trends_for_different_cluster_gamete_early_embryo_expression_time_cluster_pattern_for_correlated_DMR_Kid_parents2.pdf",trend_plot3,width =4, height =15,limitsize = FALSE)
...
head(DMR_anno2);dim(DMR_anno2)#91  8
...
#row_split = d_num,row_gap = unit(rep(2, d_num-1), "mm"),
pdf("/mnt/data/chenwei/huahua/manuscript/figure/figure4/no_remove_JTA_single_Cell_pre_implantation_embryo_rearrange_split_mean_expression_cluster_pattern_for_correlated_DMR_Kid_parents_heatmaps.pdf",width = 8,height =8)
print(htkm)
dev.off()
...
DMRs_unique<-unique(as.character(DMR_anno2$V5));length(DMRs_unique)#91
...
write.table(cor_DMR_gene, file=paste0("/mnt/data/chenwei/huahua/4.methy_result/1.DMR_called/no_remove_JTA_single_Cell_pre_implantation_embryo_overlapped_cor_DMR_gene_kids_AMA_vs_Young_200bp_DMR_myDiff15q005.txt"),quote=F,row.names=F,col.names=T,sep = "\t")
...
head(plot_matrix2);dim(plot_matrix2)# 91  7
...
pdf("/mnt/data/chenwei/huahua/manuscript/figure/figure4/no_remove_Gene_add_rearrange_split_mean_JTA_single_Cell_pre_implantation_embryo_cluster_pattern_for_correlated_DMR_Kid_parents_heatmaps.pdf",width = 8,height =8)
print(htkm)
dev.off()

###使用原始表达矩阵
data_raw<-read.table('/mnt/data/chenwei/huahua/manuscript/figure/figure4/table/no_remove_no_scale_JTA_single_Cell_pre_implantation_embryo_all_cor_DMRs_zscore_data_early_embryo_expression_time_cluster_pattern_for_correlated_DMR_Kid_parents.txt', header = T,row.names = 1)
...
head(plot_matrix3);dim(plot_matrix3)# 91  7
...
pdf("/mnt/data/chenwei/huahua/manuscript/figure/figure4/raw_data_no_remove_Gene_add_rearrange_split_mean_JTA_single_Cell_pre_implantation_embryo_cluster_pattern_for_correlated_DMR_Kid_parents_heatmaps.pdf",width = 8,height =8)
print(htkm)
dev.off()

##save as supplementary table 8 
DMR_anno4<-merge(DMR_anno2,cor_DMR_gene[,c("distanceToTSS","SYMBOL","annotation","V5")],by="V5")
head(DMR_anno4)
rownames(DMR_anno4)<-DMR_anno4$V4
head(htkm_module);head(DMR_anno4)
rownames(htkm_module)<-htkm_module$row_DMRs

setdiff(htkm_module$row_DMRs,DMR_anno4$V4)
setdiff(DMR_anno4$V4,htkm_module$row_DMRs)
intersect(DMR_anno4$V4,htkm_module$row_DMRs)
DMR_anno5<-merge(DMR_anno4,htkm_module,by=0)
DMR_anno5<-DMR_anno5[order(DMR_anno5$Cluster,decreasing = F),]
head(DMR_anno5)
write.table(DMR_anno5, file=paste0("/mnt/data/chenwei/huahua/4.methy_result/1.DMR_called/no_remove_JTA_single_Cell_pre_implantation_embryo_overlapped_cor_DMR_gene_supplementary_table_8.txt"),quote=F,row.names=F,col.names=T,sep = "\t")
...
#基于模糊c均值聚类（timeclust()参数algo='cm'）的原理对蛋白质表达值的时间序列进行了聚类。timeclust()还提供了其它的聚类算法，如层次聚类（参数algo='hc'）、k均值划分（参数algo='km'）、围绕中心点划分（参数algo='pam'）等。
#颜色、线宽、坐标轴、字体等细节可以在函数中调整，具体参数详见函数帮助
pdf("/mnt/data/chenwei/huahua/4.methy_result/1.DMR_called/no_remove_timecplot_km_JTA_single_Cell_pre_implantation_embryo_expression_pattern_for_correlated_DMRs_Kid_parents.pdf", width =24,height = 12)
...
write.table(data_expr3b,'/mnt/data/chenwei/huahua/4.methy_result/1.DMR_called/no_remove_JTA_single_Cell_pre_implantation_embryo_cluster_pattern_for_correlated_DMR_Kid_parents.txt', sep = '\t', col.names = NA, quote = FALSE)
...
write.table(data_expr3a, '/mnt/data/chenwei/huahua/manuscript/figure/figure4/table/no_remove_hc_km_zscore_data_JTA_single_Cell_pre_implantation_embryo_cluster_pattern_for_correlated_DMR_Kid_parents.txt', sep = '\t', col.names = NA, quote = FALSE)
